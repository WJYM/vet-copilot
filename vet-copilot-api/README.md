...
![image-20230315194534959](https://ktianc.oss-cn-beijing.aliyuncs.com/vet-copilot/public/images/image-20230315194534959.png)

## 使用

```
source /opt/env/vet-copilot-pro/bin/activate
...
# 线上安装更新依赖库
/opt/env/vet-copilot-pro-310/bin/pip install -r requirements.txt -i https://mirrors.aliyun.com/pypi/simple/
```
...
- 根据模型配置你的alembic：
  ...
  [dev]
  ...
  sqlalchemy.url = mysql+pymysql://your_username:password@ip:port/vet-copilot
  ...
  ```
  ```python
  # alembic/env.py
...
以常见问题表为主表，查询出创建用户名称为vet-copilot的用户，创建了哪些常见问题，并加载出用户信息：

```python
v_options = [joinedload(VadminIssue.create_user)]
v_join = [["create_user"]]
v_where = [VadminUser.name == "vet-copilot"]
